...
For more complex queries, you can use the filter query language. The examples below cover the most common patterns. For the full operator and field reference — including all comparators, filterable fields, value formatting rules, and a quick-reference example table — see [Trace query syntax: filter query language](/langsmith/trace-query-syntax#filter-query-language).
