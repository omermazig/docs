...
For Anthropic models, `create_deep_agent` automatically applies prompt caching to the stable sections of the system prompt. The base agent instructions and skill content are static or mostly stable across turns, so caching them avoids reprocessing the same tokens across calls, reducing both latency and cost on long-running agents. Dynamic sections such as memory may change between turns; the harness and middleware order content to reduce cache invalidation where possible.
