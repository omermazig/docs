...
agent = create_agent("openai:gpt-5.5", tools=[search])
```
...
agent = create_agent("openai:gpt-5.5", tools=tools, response_format=Answer)
...
```python
agent = create_agent("openai:gpt-5.5", tools=tools, name="research_assistant")
```
...
for kind, item in stream.interleave("messages", "tool_calls"):
    if kind == "messages":
        ...
        print(f"\nCalling tool: {item.tool_name}({item.input})")
```
